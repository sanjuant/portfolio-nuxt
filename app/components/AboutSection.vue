      ...
      <div class="mx-auto max-w-prose lg:max-w-none">
        ...
        >
          ...
          <span class="text-indigo-600">{{
              aboutMe?.data?.title ?? 'FirstName LastName'
            }}</span
          >,
        </h1>
      </div>
      ...
      <div class="lg:grid lg:grid-cols-2 lg:items-start lg:gap-8">
        <div class="relative z-10 mb-32 sm:mb-28 lg:mb-0">
          <div
              ...
              v-html="
              aboutMe?.data?.content ?? 'Data cannot be retrieved'
            "
          ></div>
        </div>
          ...
          >
            <div class="rounded-t-lg px-6 py-8 sm:px-10 sm:pt-10 sm:pb-8">
              <div class="flex flex-row items-center">
                <img
                    v-if="
                    !aboutMe?.data?.recommendation
                      ?.company_logo_is_colored
                  "
                    class="max-h-12 dark:invert"
                    :src="
                    formatImageUrl(
                      aboutMe?.data?.recommendation?.company_logo.data
                        ?.attributes?.url
                    ) ??
                ...
                />
                <div
                    v-if="aboutMe?.data?.recommendation?.company_title"
                    class="ml-4 text-3xl font-bold text-gray-900 dark:text-slate-100"
                    v-html="
                    aboutMe?.data?.recommendation?.company_title
                  "
                ></div>
              </div>
              <div
              ...
              >
                ...
                <p class="relative">
                  {{
                    aboutMe?.data?.recommendation?.message ??
            ...
            >
              ...
              >
                <img
                  ...
                  "
                    :alt="
                    aboutMe?.data?.recommendation?.picture?.alternativeText ?? ''
                  "
                />
              </span>
              <span
              ...
              >
                ...
                >{{
                    aboutMe?.data?.recommendation?.author ??
                    'John Doe'
                  }}</strong
                >
                <br class="sm:hidden lg:block xl:hidden" /><span
              >{{
                  aboutMe?.data?.recommendation?.company_job ??
                  'Job'
                }}
                  at
                  {{
                  aboutMe?.data?.recommendation?.company_name ??
                  'Company'
                }}</span
              >
              </span>
